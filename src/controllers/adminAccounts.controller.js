...
async function getUserDetail(req, res) {
  const { rows } = await query(
    `SELECT u.id, u.full_name, u.email, u.phone, u.bvn, u.status, u.kyc_status, u.kyc_tier,
            u.address, u.nin, u.tier_upgrade_status, u.created_at,
  ...
  res.json({ success: true, data: { ...rows[0], accountActions: actions } });
}
